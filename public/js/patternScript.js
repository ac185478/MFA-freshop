...
for (var a = 9; a >= 1; a--) {
  lockBox.insertAdjacentHTML(
    "afterbegin",
    `<div class="dot"><div class="dotArea" onmousedown="down(this)"><i>${a}</i></div></div>`
  );
}
...
function giveOut(inputData) {
  ...
  fetch("/2far/pattern", {
    ...
    body: JSON.stringify({"token":inputData}),
